	...
	tests := []struct {
	...
	}{
		{
			...
			installParams: InstallerParameters{
				URL:  "https://raw.githubusercontent.com/khulnasoft-lab/gobin/main/install.sh",
				Args: "-b /usr/local/bin",
			},
			wantMethod: githubrelease.ResolveMethod,
			wantParams: githubrelease.VersionResolutionParameters{
				Repo: "khulnasoft-lab/gobin",
			},
		},
		{
			name: "valid github.com arguments",
			installParams: InstallerParameters{
				URL:  "https://github.com/khulnasoft-lab/gobin/main/install.sh",
				Args: "-b /usr/local/bin",
			},
			wantMethod: githubrelease.ResolveMethod,
			wantParams: githubrelease.VersionResolutionParameters{
		...
		},
		{
			...
			installParams: InstallerParameters{
				URL:  "https://raw.somewhere.com/khulnasoft-lab/gobin/main/install.sh",
				Args: "-b /usr/local/bin",
			},
		...
		},
		{
			...
			installParams: map[string]string{
				"repo": "github.com/khulnasoft-lab/gobin",
			},
			wantErr: assert.Error,
		},
	}
